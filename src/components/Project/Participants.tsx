'use client'
import { markReceived, createLateJoinTransfers, confirmLateJoinReceipt } from '@/app/project/[id]/actions'
import { useTransition, useState } from 'react'

export function Participants(props: {
  projectId: string
  participants: Array<{ id: string, user_id: string, role: string, short_code: string | null }>
  preferred: Record<string, { label: string|null, value: string, type: string }>
  allOptions: Record<string, Array<{label:string|null, value:string, type:string, priority:number}>>
  paidIds: string[]
  afterDeadlineIds: string[]
  transfers: Array<any>
  ...
  for (const t of (props.transfers ?? [])) {
    ...
    arr.push(t); transfersByRecipient.set(t.to_participant_id, arr)
  }
    ...
    <section className="border rounded-xl p-4 space-y-4">
      ...
      {props.organizerId && (
        <div className="rounded border p-3 space-y-2">
          <div className="font-medium">Late-Join (Organizer)</div>
          <form onSubmit={(e)=>{e.preventDefault(); if (!newcomerId.trim()) return; start(async ()=>{ await createLateJoinTransfers(props.projectId, newcomerId.trim()); setNewcomerId('') })}}>
            ...
            <div className="text-xs opacity-60 mt-1">This will create a transfer row for each paid participant.</div>
          </form>
        </div>
      )}
      ...
      <div className="grid gap-3">
        {props.participants.map(p => {
          const pref = props.preferred[p.id]
          const all = props.allOptions[p.id] ?? []
          const paid = props.paidIds.includes(p.id)
          const late = props.afterDeadlineIds.includes(p.id)
          ...
          return (
            <div key={p.id} className="rounded border p-3 space-y-2">
              ...
              {paid && !late && <div className="text-sm text-green-700">Payment received (counts for threshold)</div>}
              {late && (
